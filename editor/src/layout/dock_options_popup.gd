...
func _on_dock_region_button_pressed(button: Control) -> void:
	LayoutManager.dock_panel(region.current_panel, button.dock_pos)
	self.hide()
...
func _on_button_make_floating_pressed() -> void:
	...
	if panel == null:
		self.hide()
		return
	var dock_pos := LayoutManager.create_panel_dock_window(region)
	LayoutManager.dock_panel(panel, dock_pos)
	self.hide()
