...
var PanelDockWindow_Scene := preload("res://src/docking/panel_dock_window.tscn")
...
func load_layout(path: String):
	...
	if err != OK:
		push_error("[DockingManager] Failed to load layout config file at %s : %s" % [path, error_string(err)])
		return
	...
	for panel_type in cfg.get_section_keys(CFG_SEC_PANELS):
		if not Panels.has(panel_type):
			push_warning("[DockingManager] Invalid panel type in layout file: %s" % panel_type)
			continue
		...
		if not dock_pos in PanelDocks:
			push_warning("[DockingManager] Invalid panel docking position: %s" % pos_str)
			continue
...
func save_layout(path: String):
	...
	if err != OK:
		push_error("[DockingManager] Failed to save layout config file at %s : %s" %[path, error_string(err)])
